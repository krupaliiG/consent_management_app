...
exports.GiveConsents = async (request, response) => {
  try {
    ...
    await data.save();
    response.status(200).send({success: true, message:"Consent added Successfullly!"});
  } catch (err) {
    response.status(400).send({success: false, message: err.message});
  }
};
...
exports.updateConsent = async (request, response) => {
  try {
    ...
    const data = await Consent.findByIdAndUpdate(ObjectId(id), {updatedBy: _id, ...updateData});

    response.status(200).send({success: true, message:"Consent updated Successfullly!"});
  } catch (err) {
    response.status(400).send({success: false, message: err.message});
  }
};
...
exports.deleteConsent = async (request, response) => {
  try {
    ...
    response.status(200).send({success: true, message:"Consent deleted Successfullly!"});
  } catch (err) {
    response.status(400).send({success: false , message: err.message});
  }
};
